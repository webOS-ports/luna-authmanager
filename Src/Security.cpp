...
void Security::registerService()
{
    ...
    if (LSRegister("com.palm.systemmanager-keymanager", &m_service, &err)) {
        if (LSGmainAttach(m_service, AuthService::instance()->mainLoop(), &err)) {
...
bool Security::matchPasscode(std::string passcode, int& retriesLeft, bool& lockedOut)
{
    ...
    char* encryptedStr = 0;
    static uint32_t lastFailure = curTimeMs();
    QByteArray hashed;
    ...
    if ((policy == 0 || pm->policyPending()) && m_numRetries == 0) {
        ...
        if (curTimeMs() - lastFailure < s_deviceLockOutDuration) {
            lockedOut = true;
            goto Done;
        }
    ...
    const std::string& passcode) const
{
    ...
    QString pass = QString::fromUtf8(passcode.c_str());
